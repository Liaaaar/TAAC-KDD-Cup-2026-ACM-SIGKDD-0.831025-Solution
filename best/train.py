...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--seq_semilocal_causal_mask', action='store_true', default=False,
                        help='Enable HSTU-style semi-local causal mask in TransformerEncoder self-attention')
    parser.add_argument('--seq_semilocal_window', type=int, default=0,
                        help='History window for semi-local causal mask; 0 means causal over all past tokens')
    ...
    parser.add_argument('--temporal_encoding_mode', type=str, default='enhanced',
                        choices=['original', 'enhanced'],
                        help='original = time bucket only; enhanced = add cross-day/time extras')
    parser.add_argument('--shared_temporal_embeddings', action='store_true', default=True,
                        help='Share time bucket/cross-day embeddings across sequence domains')
    parser.add_argument('--per_domain_temporal_embeddings',
                        dest='shared_temporal_embeddings',
                        action='store_false',
                        help='Use separate time bucket/cross-day embeddings per sequence domain')
    ...
    parser.add_argument('--ns_tokenizer_type', type=str, default='rankmixer',
                        ...
                        help='NS tokenizer variant: '
                             ...
                             'fieldaware = assign whole fid embeddings to tunable '
                             'token buckets without splitting fields, '
                             'fieldaware_shuffle = fieldaware with a deterministic '
    ...
    parser.add_argument('--fieldaware_shuffle_seed', type=int, default=42,
                        help='Seed used by fieldaware_shuffle to permute fid order '
                             'before assigning fids to token buckets')
    parser.add_argument('--user_dense_tokens', type=int, default=1,
                        help='Number of user dense NS tokens (0 = disabled)')
    parser.add_argument('--item_dense_tokens', type=int, default=1,
                        help='Number of item dense NS tokens (0 = disabled)')
    parser.add_argument('--ns_token_time_feats', type=str, default='none',
                        choices=['none', 'abs'],
                        help='Add sample-level absolute time to NS tokens: '
                             'none = disabled, abs = 12-dim continuous time projection')
    ...
    parser.add_argument('--use_target_attention', action='store_true', default=False,
                        help='Enable DIN-style target-aware attention + item-conditioned '
                             'query generation / output residual')
    parser.add_argument('--no_target_attention', dest='use_target_attention',
                        action='store_false',
                        help='Disable target-aware attention')
    parser.add_argument('--use_din_interest', action='store_true', default=False,
                        help='Enable backup-style DIN interest tokens appended to NS tokens')
    parser.add_argument('--din_mode', type=str, default=None,
                        choices=['interest', 'ref', 'both', 'none'],
                        help='Unified DIN selector: interest = backup-style DIN '
                             'NS tokens, ref = target-attention residual, '
                             'both = enable both, none = disable both. When set, '
                             'it overrides --use_target_attention and '
                             '--use_din_interest.')
    parser.add_argument('--din_hidden_mult', type=int, default=4,
                        help='Hidden dimension multiplier for DIN activation MLP')
    parser.add_argument('--din_dropout', type=float, default=0.0,
                        help='Dropout rate for DIN activation MLP')
    parser.add_argument('--din_target_source', type=str, default='item_all',
                        choices=['item_ns', 'item_all'],
                        help='Target source for DIN: item_ns or item_all')
    ...
    parser.add_argument('--use_ema', action='store_true', default=False,
                        help='Enable dense-only EMA for model weights')
    ...
    if args.din_mode is not None:
        args.use_din_interest = args.din_mode in {'interest', 'both'}
        args.use_target_attention = args.din_mode in {'ref', 'both'}
...
def main() -> None:
    ...
    model_args = {
        ...
        "temporal_encoding_mode": args.temporal_encoding_mode,
        "use_calendar_time": args.use_calendar_time,
        "shared_temporal_embeddings": args.shared_temporal_embeddings,
        ...
        "fieldaware_shuffle_seed": args.fieldaware_shuffle_seed,
        "user_dense_tokens": args.user_dense_tokens,
        "item_dense_tokens": args.item_dense_tokens,
        "abs_time_feat_dim": (
            ABS_TIME_CONT_FEAT_DIM if args.ns_token_time_feats == 'abs' else 0
        ),
        ...
        "use_din_interest": args.use_din_interest,
        "din_hidden_mult": args.din_hidden_mult,
        "din_dropout": args.din_dropout,
        "din_target_source": args.din_target_source,
    ...
    logging.info(
        ...
        f"user_ns_tokens={args.user_ns_tokens}, item_ns_tokens={args.item_ns_tokens}, "
        f"user_dense_tokens={args.user_dense_tokens}, "
        f"item_dense_tokens={args.item_dense_tokens}, "
        f"ns_token_time_feats={args.ns_token_time_feats}"
    )
