...
_FALLBACK_MODEL_CFG = {
    ...
    "shared_temporal_embeddings": True,
    ...
    # Target-aware attention / DIN / NS / temporal switches
    "use_target_attention": False,
    "use_din_interest": False,
    "din_hidden_mult": 4,
    "din_dropout": 0.0,
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        if key == "abs_time_feat_dim":
            if "abs_time_feat_dim" in train_config:
                cfg[key] = train_config["abs_time_feat_dim"]
            elif train_config.get("ns_token_time_feats") == "abs":
                cfg[key] = ABS_TIME_CONT_FEAT_DIM
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
            continue
...
def _batch_to_model_input(
    ...
    return ModelInput(
        ...
        abs_time_feats=device_batch.get("abs_time_cont_feats"),
    )
...
def main() -> None:
    ...
    # Convert model to BF16 for faster inference with lower memory
    use_bf16 = device == "cuda" and torch.cuda.is_bf16_supported()
    if use_bf16:
        model.to(torch.bfloat16)
        logging.info("Model converted to BF16 for inference")
    else:
        logging.info("BF16 not available, using default precision")
