...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        self.sparse_param_ptrs = set()
        if hasattr(model, 'get_sparse_params'):
            sparse_params = model.get_sparse_params()
            self.sparse_param_ptrs = {p.data_ptr() for p in sparse_params}
        ...
        if self.use_ema and ema_decay > 0:
            self.ema = ModelEMA(
                model,
                decay=ema_decay,
                exclude_param_ptrs=self.sparse_param_ptrs,
            )
            logging.info(
                f"Dense-only EMA enabled: decay={ema_decay}, "
                f"tracked_tensors={len(self.ema.shadow)}, "
                f"excluded_sparse_tensors={len(self.sparse_param_ptrs)}"
            )
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        elif self.loss_type == 'bce_focal02':
