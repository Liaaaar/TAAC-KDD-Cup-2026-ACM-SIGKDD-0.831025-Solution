...
class ModelEMA:
    ...
    def __init__(
        ...
        model: nn.Module,
        decay: float = 0.999,
        exclude_param_ptrs: Optional[set[int]] = None,
    ) -> None:
        self.decay = decay
        self.shadow: Dict[str, torch.Tensor] = {}
        self.backup: Dict[str, torch.Tensor] = {}
        exclude_param_ptrs = exclude_param_ptrs or set()
        for name, param in model.named_parameters():
            if param.requires_grad and param.data_ptr() not in exclude_param_ptrs:
...
def set_seed(seed: int) -> None:
    ...
    PyTorch generator and all CUDA generators, then requests deterministic
    kernels where PyTorch exposes them.
