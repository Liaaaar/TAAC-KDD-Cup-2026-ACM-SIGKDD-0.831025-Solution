...
class ModelInput(NamedTuple):
    ...
    seq_session_buckets: dict  # {domain: tensor [B, L]}
    seq_cross_day_buckets: dict  # {domain: tensor [B, L]}
    ...
    abs_time_feats: Optional[torch.Tensor] = None  # [B, 12] continuous sample time
...
class DINActivation(nn.Module):
    """Target-aware DIN activation unit that returns one interest vector."""
    ...
    def __init__(
        ...
        mid_dim = max(hidden_dim // 2, d_model)
    ...
    def forward(
        ...
        B, L, D = hist.shape
        target_expand = target.unsqueeze(1).expand(-1, L, -1)
        x = torch.cat([
            hist,
            target_expand,
            hist - target_expand,
        ...
        valid = ~mask
        all_pad = valid.sum(dim=1) == 0
        weights = torch.softmax(scores, dim=-1)
        weights = weights.masked_fill(mask, 0.0)
        weights = weights / weights.sum(dim=1, keepdim=True).clamp(min=1e-6)

        interest = torch.sum(hist * weights.unsqueeze(-1), dim=1)
        if all_pad.any():
            interest = interest.masked_fill(all_pad.unsqueeze(-1), 0.0)
        return self.out_norm(interest)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        hidden_mult: int = 4,
        dropout: float = 0.0,
        semilocal_causal_mask: bool = False,
        semilocal_window: int = 0,
    ) -> None:
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.semilocal_causal_mask = bool(semilocal_causal_mask)
        self.semilocal_window = int(semilocal_window)
    ...
    def _build_semilocal_causal_mask(
        self,
        seq_len: int,
        device: torch.device,
    ) -> Optional[torch.Tensor]:
        if not self.semilocal_causal_mask:
            return None
        ...
        blocked = k_pos > q_pos
        if self.semilocal_window > 0:
            blocked = blocked | ((q_pos - k_pos) >= self.semilocal_window)

        return blocked.float().masked_fill(blocked, float('-inf'))
    ...
    def forward(
        ...
        attn_mask = self._build_semilocal_causal_mask(
            x.shape[1],
            x.device,
        )
        x, _ = self.self_attn(
...
def create_sequence_encoder(
    ...
    Args:
        ...
        causal: Whether to use causal mask in LongerEncoder.
        semilocal_causal_mask: Whether TransformerEncoder uses a causal local
            attention mask.
        semilocal_window: Local causal window size. 0 keeps all history.
    ...
    if encoder_type == 'swiglu':
        ...
        return TransformerEncoder(
            ...
            hidden_mult,
            dropout,
            semilocal_causal_mask=semilocal_causal_mask,
            semilocal_window=semilocal_window,
        )
...
class FieldAwareNSTokenizer(nn.Module):
    """NS tokenizer that never splits a field embedding across tokens.

    It keeps RankMixer's configurable token count, but assigns complete fid
    embeddings to token buckets before concatenation/projection.
    """
    ...
    def __init__(
        ...
        if not all_fids:
            raise ValueError("FieldAwareNSTokenizer requires at least one fid")
        if shuffle_seed is not None:
            all_fids = list(all_fids)
            random.Random(int(shuffle_seed)).shuffle(all_fids)
            logging.info(
                f"FieldAwareNSTokenizer: shuffled fid order with seed={shuffle_seed}"
            )

        base_bucket_size = len(all_fids) // num_ns_tokens
        remainder = len(all_fids) % num_ns_tokens
        self.token_fid_buckets = []
        start = 0
        for i in range(num_ns_tokens):
            size = base_bucket_size + (1 if i < remainder else 0)
            end = start + size
        ...
        for bucket in self.token_fid_buckets:
            ...
            self.token_projs.append(nn.Sequential(
                nn.Linear(in_dim, d_model),
                nn.LayerNorm(d_model),
            ))

        bucket_sizes = [len(bucket) for bucket in self.token_fid_buckets]
        logging.info(
            f"FieldAwareNSTokenizer: {len(all_fids)} fids, "
            f"num_ns_tokens={num_ns_tokens}, bucket_sizes={bucket_sizes}"
        )
    ...
    def _embed_fid(self, int_feats: torch.Tensor, fid_idx: int) -> torch.Tensor:
        ...
        if emb_real_idx == -1:
            ...
            if hash_key in self._hash_embs:
                ...
                dtype = self.token_projs[0][0].weight.dtype
                return torch.zeros(
                    int_feats.shape[0],
                    self.emb_dim,
        ...
        for bucket, proj in zip(self.token_fid_buckets, self.token_projs):
            if bucket:
                bucket_emb = torch.cat([
                    self._embed_fid(int_feats, fid_idx)
                    for fid_idx in bucket
                ], dim=-1)
            else:
                dtype = proj[0].weight.dtype
                bucket_emb = torch.zeros(
                    int_feats.shape[0],
                    1,
                    device=int_feats.device,
                    dtype=dtype,
...
class DenseFeatureTokenizer(nn.Module):
    """Project a flat dense vector into one or more NS tokens."""
    ...
    def __init__(
        ...
        self._pad_size = self.padded_total_dim - self.dense_dim
        self.token_projs = nn.ModuleList([
            nn.Sequential(
        ...
        logging.info(
            f"DenseFeatureTokenizer: dense_dim={dense_dim}, "
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        seq_semilocal_window: int = 0,
        action_num: int = 1,
        num_time_buckets: int = 65,
        num_time_span_buckets: int = 0,
        temporal_encoding_mode: str = 'enhanced',
        use_calendar_time: bool = True,
        shared_temporal_embeddings: bool = True,
        rank_mixer_mode: str = 'full',
        use_rope: bool = False,
        ...
        fieldaware_shuffle_seed: int = 42,
        user_dense_tokens: int = 1,
        item_dense_tokens: int = 1,
        abs_time_feat_dim: int = 0,
        # DCN-v2 cross network
        ...
        use_din_interest: bool = False,
        din_hidden_mult: int = 4,
        din_dropout: float = 0.0,
        din_target_source: str = 'item_all',
        ...
        if temporal_encoding_mode not in ['original', 'enhanced']:
            raise ValueError(f"Unknown temporal_encoding_mode: {temporal_encoding_mode}")
        self.temporal_encoding_mode = temporal_encoding_mode
        self.use_enhanced_temporal = temporal_encoding_mode == 'enhanced'
        ...
        self.use_din_interest = bool(use_din_interest)
        self.din_target_source = din_target_source
        if din_target_source not in ['item_ns', 'item_all']:
            raise ValueError(f"Unknown din_target_source: {din_target_source}")
        self.use_temporal_bias = use_temporal_bias
        self.use_time_gap = use_time_gap
        self.user_dense_tokens = int(user_dense_tokens)
        self.item_dense_tokens = int(item_dense_tokens)
        self.abs_time_feat_dim = int(abs_time_feat_dim)
        if self.user_dense_tokens < 0:
            raise ValueError(f"user_dense_tokens must be non-negative, got {user_dense_tokens}")
        if self.item_dense_tokens < 0:
            raise ValueError(f"item_dense_tokens must be non-negative, got {item_dense_tokens}")
        ...
        elif ns_tokenizer_type in {'fieldaware', 'fieldaware_shuffle'}:
            # Configurable NS token count while keeping each fid embedding whole.
            if user_ns_tokens <= 0:
                user_ns_tokens = len(user_ns_groups)
            if item_ns_tokens <= 0:
                item_ns_tokens = len(item_ns_groups)
            shuffle_user_seed = (
                int(fieldaware_shuffle_seed)
                if ns_tokenizer_type == 'fieldaware_shuffle'
                else None
            )
            shuffle_item_seed = (
                int(fieldaware_shuffle_seed) + 1
                if ns_tokenizer_type == 'fieldaware_shuffle'
                else None
            )
            self.user_ns_tokenizer = FieldAwareNSTokenizer(
                feature_specs=user_int_feature_specs,
                groups=user_ns_groups,
                emb_dim=emb_dim,
                d_model=d_model,
                ...
                emb_skip_threshold=emb_skip_threshold,
                hash_bucket_size=hash_bucket_size,
                shuffle_seed=shuffle_user_seed,
            )
            num_user_ns = user_ns_tokens

            self.item_ns_tokenizer = FieldAwareNSTokenizer(
                feature_specs=item_int_feature_specs,
                groups=item_ns_groups,
                emb_dim=emb_dim,
                d_model=d_model,
                ...
                emb_skip_threshold=emb_skip_threshold,
                hash_bucket_size=hash_bucket_size,
                shuffle_seed=shuffle_item_seed,
            )
            num_item_ns = item_ns_tokens
        else:
        ...
        self.has_user_dense = user_dense_dim > 0 and self.user_dense_tokens > 0
        self.user_sparse_dense_pair_residual = None
        if self.has_user_dense:
            if self.user_dense_tokens == 1:
                self.user_dense_proj = nn.Sequential(
                    nn.Linear(self.user_dense_proj_dim, d_model),
                    nn.LayerNorm(d_model),
                )
            else:
                self.user_dense_proj = DenseFeatureTokenizer(
                    dense_dim=self.user_dense_proj_dim,
        ...
        self.has_item_dense = item_dense_dim > 0 and self.item_dense_tokens > 0
        if self.has_item_dense:
            if self.item_dense_tokens == 1:
                self.item_dense_proj = nn.Sequential(
                    nn.Linear(item_dense_dim, d_model),
                    nn.LayerNorm(d_model),
                )
            else:
                self.item_dense_proj = DenseFeatureTokenizer(
        ...
        self.has_abs_time = self.abs_time_feat_dim > 0
        if self.has_abs_time:
            self.abs_time_proj = nn.Sequential(
                nn.Linear(self.abs_time_feat_dim, d_model),
                nn.LayerNorm(d_model),
            )
        ...
        if self.use_din_interest:
            self.num_ns += self.num_sequences

        if self.use_din_interest:
            self.din_layers = nn.ModuleList([
                DINActivation(
                    d_model=d_model,
            ...
            self.din_layers = None

        logging.info(
            f"DIN interest enabled={self.use_din_interest}, "
            f"din_target_source={self.din_target_source}, "
            f"extra_din_tokens={self.num_sequences if self.use_din_interest else 0}"
        )
        logging.info(f"Total NS tokens: {self.num_ns}")
        ...
        # ================== Time Interval / Cross-Day Embeddings (optional) ==================
        self.shared_temporal_embeddings = shared_temporal_embeddings
        if shared_temporal_embeddings:
            if num_time_buckets > 0:
                self.time_embedding = nn.Embedding(
                    num_time_buckets, d_model, padding_idx=0)
            if num_time_buckets > 0 and self.use_enhanced_temporal:
            ...
            if num_time_buckets > 0 and self.use_enhanced_temporal:
                self.session_embeddings = nn.ModuleDict({
                    d: nn.Embedding(17, d_model, padding_idx=0)
                    for d in self.seq_domains
                })
                self.cross_day_embeddings = nn.ModuleDict({
                    d: nn.Embedding(8, d_model, padding_idx=0)
                    for d in self.seq_domains
                })
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                top_k=seq_top_k,
                causal=seq_causal,
                semilocal_causal_mask=seq_semilocal_causal_mask,
                semilocal_window=seq_semilocal_window,
    ...
    def _init_params(self) -> None:
        ...
        if self.num_time_buckets > 0:
            if self.shared_temporal_embeddings:
                ...
                for emb in self.time_embeddings.values():
                    nn.init.xavier_normal_(emb.weight.data)
                    emb.weight.data[0, :] = 0
        if self.num_time_buckets > 0 and self.use_enhanced_temporal:
            if self.shared_temporal_embeddings:
                ...
                for emb in self.session_embeddings.values():
                    nn.init.xavier_normal_(emb.weight.data)
                    emb.weight.data[0, :] = 0
                for emb in self.cross_day_embeddings.values():
                    nn.init.xavier_normal_(emb.weight.data)
                    emb.weight.data[0, :] = 0
    ...
    def reinit_high_cardinality_params(
        ...
        if self.num_time_buckets > 0:
            skip_count += 1 if self.shared_temporal_embeddings else len(self.seq_domains)
        if self.num_time_buckets > 0 and self.use_enhanced_temporal:
            skip_count += 1 if self.shared_temporal_embeddings else len(self.seq_domains)
        if self.use_calendar_time:
            skip_count += 2
    ...
    def _get_temporal_embeddings(self, domain: str):
        """Return (time_emb, session_emb, cross_day_emb) for a sequence domain."""
        if self.shared_temporal_embeddings:
            time_emb = getattr(self, 'time_embedding', None)
            session_emb = getattr(self, 'session_embedding', None)
            cross_day_emb = getattr(self, 'cross_day_embedding', None)
        else:
            time_emb = (
                self.time_embeddings[domain]
                if hasattr(self, 'time_embeddings') and domain in self.time_embeddings
                else None
            )
            session_emb = (
                self.session_embeddings[domain]
                if hasattr(self, 'session_embeddings') and domain in self.session_embeddings
                else None
            )
            cross_day_emb = (
                self.cross_day_embeddings[domain]
                if hasattr(self, 'cross_day_embeddings') and domain in self.cross_day_embeddings
                else None
            )
        return time_emb, session_emb, cross_day_emb
    ...
    def _project_dense_tokens(
        self,
        dense_feats: torch.Tensor,
        proj: nn.Module,
    ) -> torch.Tensor:
        tokens = proj(dense_feats)
        if tokens.dim() == 2:
            tokens = F.silu(tokens).unsqueeze(1)
        else:
            tokens = F.silu(tokens)
        return tokens
    ...
    def _make_user_dense_tokens(
        ...
        """Build user dense token(s), adding sparse/dense residual to the first token."""
        dense_proj_input = self._make_user_dense_proj_input(user_dense_feats)
        user_dense_tokens = self._project_dense_tokens(
            dense_proj_input,
            self.user_dense_proj,
        )
        if self.user_sparse_dense_pair_residual is not None:
            residual = self.user_sparse_dense_pair_residual(
                user_int_feats, user_dense_feats).unsqueeze(1)
            user_dense_tokens = torch.cat(
                [user_dense_tokens[:, :1, :] + residual, user_dense_tokens[:, 1:, :]],
                dim=1,
            )
        return user_dense_tokens

    def _add_abs_time_to_ns_tokens(
        self,
        ns_tokens: torch.Tensor,
        inputs: ModelInput,
    ) -> torch.Tensor:
        """Add sample-level 12-dim absolute time projection to every NS token."""
        if not self.has_abs_time:
            return ns_tokens
        if inputs.abs_time_feats is None:
            abs_time_feats = inputs.user_int_feats.new_zeros(
                inputs.user_int_feats.shape[0],
                self.abs_time_feat_dim,
                dtype=torch.float,
            )
        else:
            abs_time_feats = inputs.abs_time_feats
        return ns_tokens + F.silu(self.abs_time_proj(abs_time_feats)).unsqueeze(1)
    ...
    def _build_din_target(
        self,
        item_ns_tokens: torch.Tensor,
        item_dense_tokens: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Build target item representation for DIN interest tokens."""
        if self.din_target_source == 'item_ns':
            target_tokens = item_ns_tokens
        elif item_dense_tokens is not None:
            target_tokens = torch.cat([item_ns_tokens, item_dense_tokens], dim=1)
        else:
            target_tokens = item_ns_tokens
        return target_tokens.mean(dim=1)

    def _append_din_interest_tokens(
        self,
        ns_parts: list,
        item_ns_tokens: torch.Tensor,
        item_dense_tokens: Optional[torch.Tensor],
        seq_tokens_list: list,
        seq_masks_list: list,
    ) -> None:
        """Append one target-aware DIN interest token per sequence domain."""
        if not self.use_din_interest:
            return

        target = self._build_din_target(
            item_ns_tokens=item_ns_tokens,
            item_dense_tokens=item_dense_tokens,
        )
        din_tokens = []
        for i in range(self.num_sequences):
            din_i = self.din_layers[i](
                target=target,
                hist=seq_tokens_list[i],
                mask=seq_masks_list[i],
            )
            din_tokens.append(din_i.unsqueeze(1))
        ns_parts.extend(din_tokens)
    ...
    def _embed_seq_domain(
        ...
        # Add time bucket and cross-day embeddings.
        time_emb, session_emb, cross_day_emb = self._get_temporal_embeddings(domain)
        if time_emb is not None:
            token_emb = token_emb + time_emb(time_bucket_ids)
        if session_emb is not None and session_bucket_ids is not None:
            token_emb = token_emb + session_emb(session_bucket_ids.clamp(0, 16))
        if cross_day_emb is not None and cross_day_bucket_ids is not None:
            token_emb = token_emb + cross_day_emb(cross_day_bucket_ids.clamp(0, 7))
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        item_dense_tok = None
        if self.has_user_dense:
            user_dense_tok = self._make_user_dense_tokens(
                inputs.user_int_feats, inputs.user_dense_feats)
            ns_parts.append(user_dense_tok)
        ns_parts.append(item_ns)
        if self.has_item_dense:
            item_dense_tok = self._project_dense_tokens(
                inputs.item_dense_feats, self.item_dense_proj)
        ...
        for domain in self.seq_domains:
            ...
            tokens = self._embed_seq_domain(
                ...
                session_bucket_ids=inputs.seq_session_buckets.get(domain),
                cross_day_bucket_ids=inputs.seq_cross_day_buckets.get(domain),
                time_gap=tg,
        ...
        self._append_din_interest_tokens(
            ns_parts=ns_parts,
            item_ns_tokens=item_ns,
            item_dense_tokens=item_dense_tok,
            seq_tokens_list=seq_tokens_list,
            seq_masks_list=seq_masks_list,
        )
        ns_tokens = torch.cat(ns_parts, dim=1)  # (B, num_ns, D)
        ns_tokens = self._add_abs_time_to_ns_tokens(ns_tokens, inputs)
