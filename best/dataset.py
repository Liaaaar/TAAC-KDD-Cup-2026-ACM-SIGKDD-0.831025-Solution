...
MAX_SESSION_BUCKET = 16
NUM_SESSION_BUCKETS = MAX_SESSION_BUCKET + 1

ABS_TIME_CONT_PERIODS = np.array([
    3600,       # hour
    86400,      # day
    604800,     # week
    2592000,    # 30 days
...
ABS_TIME_CONT_FEAT_DIM = 2 * len(ABS_TIME_CONT_PERIODS) + 2
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            seed: base seed used for deterministic buffer shuffling.
        """
        super().__init__()
        ...
        self._buf_abs_time_cont_feats = np.zeros(
            (B, ABS_TIME_CONT_FEAT_DIM), dtype=np.float32)
        self._buf_seq = {}
        self._buf_seq_tb = {}
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        iter_seed = self.seed + worker_id * 1000003 + self._iter_count
        self._iter_count += 1
    ...
    def _flush_buffer(
        ...
        rand_idx = (
            torch.randperm(total_rows, generator=generator)
            if self.shuffle
    ...
    def _build_abs_time_cont_feats(
        self,
        timestamps: "npt.NDArray[np.int64]",
    ) -> "npt.NDArray[np.float32]":
        """Build 12 continuous absolute-time features from sample timestamps."""
        B = timestamps.shape[0]
        feats = self._buf_abs_time_cont_feats[:B]
        feats[:] = 0.0

        ts = timestamps.astype(np.float32).reshape(-1, 1)
        phases = ts / ABS_TIME_CONT_PERIODS.reshape(1, -1)
        n_period = len(ABS_TIME_CONT_PERIODS)
        feats[:, :n_period] = np.sin(2.0 * np.pi * phases)
        feats[:, n_period:2 * n_period] = np.cos(2.0 * np.pi * phases)

        seconds_in_day = np.mod(timestamps, 86400).astype(np.float32)
        base = 2 * n_period
        feats[:, base] = seconds_in_day / 86400.0
        feats[:, base + 1] = (seconds_in_day // 3600).astype(np.float32) / 23.0
        return feats.copy()
    ...
    def _build_session_features(
        ...
        """Build per-sequence session-rank and cross-day visit buckets."""
        B, L = ts_padded.shape
        valid = ts_padded > 0

        session_bucket = np.zeros((B, L), dtype=np.int64)
        if L > 0:
            gaps = np.zeros((B, L), dtype=np.int64)
            gaps[:, 1:] = np.maximum(ts_padded[:, :-1] - ts_padded[:, 1:], 0)
            new_session = valid & (gaps > SESSION_GAP_SECONDS)
            new_session[:, 0] = valid[:, 0]
            session_rank = np.cumsum(new_session, axis=1)
            session_bucket = np.clip(
                session_rank, 0, MAX_SESSION_BUCKET
            ).astype(np.int64)
            session_bucket[~valid] = 0

        current_day = np.floor_divide(timestamps, 86400).reshape(-1, 1)
        event_day = np.floor_divide(ts_padded, 86400)
        day_delta = np.maximum(current_day - event_day, 0)
        cross_day = np.zeros((B, L), dtype=np.int64)
        cross_day[(valid) & (day_delta == 0)] = 1
        cross_day[(valid) & (day_delta == 1)] = 2
        cross_day[(valid) & (day_delta == 2)] = 3
        cross_day[(valid) & (day_delta >= 3) & (day_delta <= 6)] = 4
        cross_day[(valid) & (day_delta >= 7) & (day_delta <= 13)] = 5
        cross_day[(valid) & (day_delta >= 14) & (day_delta <= 29)] = 6
        cross_day[(valid) & (day_delta >= 30)] = 7
        return session_bucket, cross_day
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        result = {
            ...
            'abs_time_cont_feats': torch.from_numpy(
                self._build_abs_time_cont_feats(timestamps)),
            'user_id': user_ids,
        ...
        for domain in self.seq_domains:
            ...
            if ts_ci is not None:
                ...
                time_bucket[:] = buckets
                session_ids, cross_day_ids = self._build_session_features(
                    ts_padded, timestamps)
                session_bucket[:] = session_ids
                cross_day_bucket[:] = cross_day_ids

            result[f'{domain}_time_bucket'] = torch.from_numpy(time_bucket.copy())
            result[f'{domain}_session_bucket'] = torch.from_numpy(
                session_bucket.copy())
            result[f'{domain}_cross_day_bucket'] = torch.from_numpy(
                cross_day_bucket.copy())
